...
const UNIPILE_API_KEY = process.env.UNIPILE_API_KEY || process.env.VITE_UNIPILE_API_KEY;

// Validate required environment variables
if (!supabaseUrl || !supabaseAnonKey) {
  console.error('❌ Missing Supabase configuration. Required: SUPABASE_URL, SUPABASE_ANON_KEY');
  process.exit(1);
}

if (!UNIPILE_API_KEY) {
  console.error('❌ Missing UNIPILE_API_KEY environment variable');
  process.exit(1);
}

if (!supabaseServiceKey) {
  console.warn('⚠️  No SUPABASE_SERVICE_KEY provided. Admin operations may fail.');
}
...
console.log(`   Unipile API: ${UNIPILE_API_URL}`);
console.log(`   API Key configured: ${UNIPILE_API_KEY ? '✅' : '❌'}`);
